...
uint8_t rectangle_num_2 = 4;
uint8_t pass_rectangle_num_2 = 0;
...
uint8_t task4Control(CommWithGroundStation& GroundStationComm, CommWithMCU& MCUComm, cv::VideoCapture& cap_up, cv::VideoCapture& cap_down, Yolov8Custom& yolov8, Yolov5& yolov5) 
{
    ...
    std::cout << "pass_rectangle_num_2 is ---------------> " << pass_rectangle_num_2 << std::endl;
    ...
    switch(rovAutoControlState)
    {
        ...
        case -2:
        {
            ...
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[8] = 0;  // mode
            auto currentTime = std::chrono::steady_clock::now();
            if(std::chrono::duration_cast<std::chrono::seconds>(currentTime - stateEntryTime).count() >= 4) 
            {
        ...
        case 8:
        {
            ...
            {
                pass_rectangle_num_2 ++;
                if(pass_rectangle_num_2 == rectangle_num_2)
                {
                    rovAutoControlState = 20;
                }
                else
                {
                    rovAutoControlState = 2;
        ...
        case 21:
        {
            static float delay_time = 10;
            static int forward_speed_set = 0;
            static int right_spedd_set = -300;

            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[1] = 0/ 80.0 * 520; //yaw_vel
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[2] = 0 / 30.0 * 520; //depth vel
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[3] = right_spedd_set / 1000.0 * 520; //right
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[4] = (forward_speed_set + ZERO_FOREARD_THR) / 1000.0 * 520; //forward
            // GroundStationComm.GroundStationRemote.OriginalRemoteChannel[5] = 400;
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[6] = 0 / 80.0 * 520;   //roll speed
            GroundStationComm.GroundStationRemote.OriginalRemoteChannel[8] = 0;  // mode
